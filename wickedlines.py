...
DEFAULT_HUNTER_CONFIG = HunterConfig(MIN_GAMES=200, MIN_REACH_PCT=0.1, DELTA_EV_THRESHOLD=20.0, P_VALUE_THRESHOLD=0.05, MAX_DEPTH=20, BRANCH_FACTOR=3, ELO_PER_POINT=8)
            ...
            for file in files:
                if file.endswith('.md'):
                    ...
                    if len(parts) >= 3:
                        ...
                        if line_name == "start_pos": line_name = "From Starting Position"
...
def print_final_summary(args, hunt_duration):
    ...
    for i, item in enumerate(sorted_report):
        ...
        terminal_output.append(f"{rank}. {colorize(elo_gain_str, Colors.GREEN)} | {colorize(reach_pct_str, Colors.YELLOW)}")
        terminal_output.append(f"   Line: {line_str_color} {opening_str}")
        terminal_output.append(f"   Impact: Line EV: {colorize_ev(item['line_ev']):<14} | ΔEV: {delta_ev_str}")
        terminal_output.append(f"   URL:  {generate_lichess_url(item['line_moves'])}")
